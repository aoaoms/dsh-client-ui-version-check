...
The details modal shows the installed version and newer releases with their Simplified-Chinese summaries, themed to match the host UI.
...
- **Details modal**: fetches the GitHub releases API (CORS-open), filters to the installed version and newer (newest first), extracts each release's Simplified-Chinese summary, and renders it in a centered, theme-aware modal. Closes via the close button, a backdrop click, or Escape.
