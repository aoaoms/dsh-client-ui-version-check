window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		* - version data comes from a raw POST to /api/host.describe (bypassing the
		*   core client-side schema, which would strip the plugin-added fields),
		*   with a 60s cache that Refresh bypasses;
		...
		*/
		...
		/** Raw POST to /api/host.describe, bypassing the core client schema. */
		...
		/** Extract the Simplified-Chinese section of a release body. */
		function zhSummary(body) {
			const m = String(body ?? "").match(/<h3 id="cn[^"]*">([\s\S]*?)(?=<h3 id="en|$)/);
		...
		function setPopoverContent(value) {
			...
			versionRow.appendChild(muted(value.version ?? texts.loading, {}));
			...
			if (latest === null || latest === undefined) {
				...
				latestRow.appendChild(muted(latest, {}));
				const same = latest === value.version;
				latestRow.appendChild(document.createTextNode(" · "));
				latestRow.appendChild(muted(same ? texts.upToDate : texts.update, same ? STATUS_OK_STYLE : STATUS_UPDATE_STYLE));
			}
